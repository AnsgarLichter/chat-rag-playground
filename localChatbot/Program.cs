...
while (true)
{
    ...
    Console.WriteLine("Question received, searching similar documents.");
    var similarDocuments = await vectorCollection.GetSimilarDocuments(embeddingModel, question, amount: 5);
    Console.WriteLine("Similar documents found, asking the LLM.");
    var answer = await chatModel.GenerateAsync(
        $"""
    ...
    foreach (var chatResponse in answer)
    {
